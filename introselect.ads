--  Introselect — Ada/SPARK Level 4 educational package for Musser
--  introspective selection: hybrid of Quickselect + median-of-medians
--  (BFPRT) fallback (David Musser, 1997). Practical average ~O(n);
--  worst-case O(n) intent via MoM pivot when the introspective depth
--  budget is exhausted. O(1) extra space for Select_Kth aside from
--  MoM recursion O(log n).
--
--  SPARK port of Ada-Introselect: hard Max_N bound, no exceptions,
--  In_Bounds / Is_Kth_Partitioned contracts replace Invalid_Argument.
--  Non-SPARK sibling allows arbitrary A'First, Max_N = 100_000, and
--  raises on oversized / empty / bad K; this port requires A'First = 1,
--  nonempty A for Select_Kth, and uses Pre => In_Bounds (A) and then
--  K in 1 .. A'Length. Full multiset / permutation equality is verified
--  by tests rather than claimed as a Level-4 postcondition (the
--  partition / order-statistic property is proved).
--
...
is
   ...
   --  educational introselect postcondition and Level-4-provable when the
   ...
   --  Target := K (since First = 1). Lo := 1; Hi := A'Last.
   ...
   --    1. If depth = 0: BFPRT median-of-medians (groups of 5) pivot
   --       index on A(Lo .. Hi); swap that element to Hi; mark fallback.
   --    2. Else: if Hi - Lo >= 2, median-of-three on A(Lo), A(Mid),
   --       A(Hi); park the median at Hi; Depth := Depth - 1.
   --    3. Lomuto-partition A(Lo .. Hi) around A(Hi); pivot lands at P.
   --       Afterward A(Lo .. P-1) <= A(P) <= A(P+1 .. Hi).
   --    4. If P = Target, stop. If P > Target, Hi := P-1; else Lo := P+1.
   --    5. After a MoM fallback, restore depth ← 2⌊log₂(Hi-Lo+1)⌋.
   ...
   --  In-place introselect: rearranges A so A(A'First + K - 1) holds the
